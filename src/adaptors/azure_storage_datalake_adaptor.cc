...
AzureStorageDataLakeAdaptor::AzureStorageDataLakeAdaptor(const std::string& account, const std::string& filesystem, const std::string& account_key) :
...
int AzureStorageDataLakeAdaptor::read(const std::string& path, char* buff, size_t size, size_t offset) {
...
int AzureStorageDataLakeAdaptor::list(const std::string& path, std::vector<DirectoryEntry>& directory_entries, std::string& continuation_token) {
    auto container_client = Azure::Storage::Blobs::BlobContainerClient(m_blob_container_url, m_key_credential);
